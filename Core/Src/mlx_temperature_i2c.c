...
/**
  * @brief  Function to write data to temperature sensor
...
void MLX90614_WriteReg(uint8_t devAddr, uint8_t regAddr, uint16_t data)
{
	...
	HAL_I2C_Master_Transmit(I2C_DEFINITION, (devAddr << 1), i2cdata, 4, 0xFFFF);
	HAL_Delay(10);
...
/**
  * @brief  Function to read data of temperature sensor register
  * @params	devAddr: slave Address
...
/**
  * @brief  Function to read temperature sensor data after conversion
  * @params	devAddr: slave Address
  * 		regAddr : Register address to read
...
/**
  * @brief  Function to scan the devices connected.
  * @params	NOTHING
  * @retval NOTHING
  */
void MLX90614_ScanDevices (void)
{
	...
	for (int i = 0; i<128; i++)
		  {
			  result = HAL_I2C_IsDeviceReady(I2C_DEFINITION, (uint16_t) (i<<1), 2, 2);
			  if (result != HAL_OK)
			  {
				  sprintf(temp_buff, ".");
